...
def run(tf, x, u, m, s, z, v_data):
    ...
    traj = go.Figure()
    traj.add_trace(go.Scatter3d(z=r[0, :], x=r[1, :], y=r[2, :], mode='lines', name='Flight Path'))
    ...
    fig.add_trace(go.Scatter(x=t, y=np.full_like(t, v_data['velocity_max']),
                              mode='lines', name='Maximum Velocity'), row=1, col=1)

    fig.add_trace(go.Scatter(x=t, y=r[0, :], 
                             mode='lines', name='Altitude (m)'), row=1, col=2)

    fig.add_trace(go.Scatter(x=t, y=m,
                             mode='lines', name='Mass (kg)'), row=1, col=3)

    fig.add_trace(go.Scatter(x=t, y=Th, 
                             mode='lines', name='Thrust (N)'), row=2, col=1)
    fig.add_trace(go.Scatter(x=t, y=np.full_like(t, v_data['thrust_max']),
                              mode='lines', name='Maximum Thrust'), row=2, col=1)
    fig.add_trace(go.Scatter(x=t, y=Th_, 
                             mode='lines', name='Modified Thrust (N)'), row=2, col=1)

    u_angle = [np.degrees(math.acos(min(1, ui[0] / np.linalg.norm(ui)))) for ui in u.T]
    fig.add_trace(go.Scatter(x=t, y=np.full_like(t, np.degrees(v_data['angle_pt'])),
                              mode='lines', name='Desired Thrust Angle'), row=2, col=2)
    fig.add_trace(go.Scatter(x=t, y=u_angle, mode='lines', name='Thrust Angle'), row=2, col=2)
    ...
    fig.add_trace(go.Scatter(x=t, y=s, mode='lines', name='Sigma Slack'), row=2, col=3)

    fig.update_layout(title_text='Rocket Data', showlegend=False)
    fig.update_layout(height=800)  # Adjust height to fit in 2 rows
